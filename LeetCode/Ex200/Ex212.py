ass Ex212(object):
    def is_valid(self, board, cur, i, j, words, cand, res):
        #cur = []
        '''
        for (i, j) in path:
           cur += board[i][j] 
        '''
        #print 61, cur_w, board[i][j]
        #cur = cur_w + [board[i][j]]
        #cur = cur_w
        #cur = ''.join(cur)
        ...
        for w in cand2:
            if n >= len(w):
                cand.remove(w)
                continue
            if len(w) == n + 1 and board[i][j] == w[n]:
                #print 73, cur, cand2, res
                if w not in res:
                    res.append(w)
                    #cand.remove(w)
                    #to_remove = w
    ...
    def test(self, board, i, j, path2, cur_w, words, cand, res, max_l):
        if len(cur_w) > max_l:
            return False
        ...
        t = self.is_valid(board, cur_w, i, j, words, cand, res)
        if t == 0:
            #path.pop()
            #cur_w.pop()
            return False
        cur_w = cur_w[:]
        cur_w.append(board[i][j])
        #path = path[:]
        #path.append((i, j))
        path2 = set(path2)
        path2.add((i, j))
        ...
        #board[i][j] = 0
        if j < len(board[0]) - 1:
            self.test(board, i, j+1, path2, cur_w, words, cand, res, max_l)
            
        if i < len(board) - 1:
            self.test(board, i+1, j, path2, cur_w, words, cand, res, max_l)
    ...
    def findWords(self, board, words):
        ...
        for w in words:
            ...
            if len(w) < min_l:
                min_l = len(w)
        if len(board[0]) * len(board) < min_l:
            return res
            
        for i in range(m):
            for j in range(n):                
                self.test(board, i, j, set(), [], words, words[:], res, max_l)
...
#print 212, ex212.is_valid(['e', 'a', 't', ''], words)
board = ["aaaa","aaaa","aaaa","aaaa","bcde","fghi","jklm","nopq","rstu","vwxy","zzzz"]
...
#print 212, ex212.findWords(board, words), 
